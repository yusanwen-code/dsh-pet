...
dsh-pet is a native, customizable companion layer for DeepSeek Harness. It translates session activity into a stable five-state protocol and renders a declarative pet pack inside the official Web UI slot system. Version 0.1 is intentionally read-only: capability metadata can be declared, but pets cannot invoke Harness actions.
